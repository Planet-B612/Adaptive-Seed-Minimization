...
class Argument{
public:
	double eta=20000;		// the percentage of total node number taken as quota.
    ...
    int cost_type=1; // 0 for random cost, 1 for 0.01 degree, 2 for uniform cost
    ...
    int dataset_No= 18;
    int cur_dataset_No=8;
    double delta_amp=1.0;
...
void Run(int argn, char **argv)
{
    ...
    {
        if (argv[i] == string("-help") || argv[i] == string("--help"))
        {
        ...
        if( argn == 1)
        {
            cout<<"no parameters are imported, use default parameters"<<endl;
            break;
        }
		...
		if (argv[i] == string("-model"))
			{
                arg.model = argv[i + 1]== string("LT") ? "LT" : "IC";
                cout<<arg.model<<", "<<argv[i + 1]<<endl;
            }
    ...
    // for(auto k:arg.dataset_No)
    {
        ...
        {           
            ...
            arg.eta*= g.n;
            cout<<"# The parameters are: Aiming eta="<<arg.eta<<", model="<<arg.model<<", epsilon_Inf="<<arg.epsilon<<", cost_type="<<to_string(arg.cost_type)<<", batch="<<arg.batch<<", time="<<arg.time<<", delta_amp = "<<arg.delta_amp<<endl;
            ...
            if(arg.cost_type==0)
            {
                cost_file="/data/fc/graphInfo/"+arg.dataset[k]+"_cost_Rand.txt";
                std::ifstream inFile;
            ...
            {
                cost_file="/data/fc/graphInfo/"+arg.dataset[k]+"_cost_001DEG.txt";
                // cout << cost_file << endl;
                std::ifstream inFile;
                inFile.open(cost_file);
                if(!inFile)
